...
namespace Controllers.StackManager
{
    public class StackController : MonoBehaviour
    {
        ...
        private StackData GetStackData() => Resources.Load<CD_StackData>("Data/CD_StackData").StackData;
            ...
            else
            {
                ...
                else
                {
                    if (!_trigger)
                    {
                        ...
                        CoreGameSignals.Instance.onPlayerGameChange?.Invoke(Enums.GameStates.Idle);
        ...
        public void MoveStack()
        {
            ...
            for (int i = 1; i <= Count - 1; i++)
            {
                ...
                float lerpObjx = Mathf.Lerp(StackListObj[i].transform.localPosition.x, stackPos.x, 0.2f);
                float lerpobjz = Mathf.Lerp(StackListObj[i].transform.localPosition.z - StackData.CollectableOffsetInStack, stackPos.z, 0.2f);
                float lerpobjy = Mathf.Lerp(StackListObj[i].transform.localPosition.y, stackPos.y, 0.2f);
        ...
        public void HelicopterPlatformStack()
        {
            ...
            if (StackListObj.Count > 0)
            {
                ...
                if (StackListObj.Count == 0)
                {
                    ...
                    DOVirtual.DelayedCall(5.5f, () => StartCoroutine(SlowlyStackAdd()));
                }
            }
        }
        ...
        public void SlowStackStriking(int index)
        {
            for (int i = 0; i < index; i++)
            {
                ...
                PoolListObj[i].GetComponent<Animator>().SetTrigger("StandingToCrouched");
        ...
        public IEnumerator SlowlyStackAdd()
        {
            ...
            for (int i = 0; i < index + index; i++)
            {
                ...
                _slowStack[0].GetComponent<Animator>().SetTrigger("Runner");
